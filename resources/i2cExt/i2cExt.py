	...
	for eqt in Eqts:
		if eqt.status != 0:
			...
			if eqt.inputChanged == 1:
				...
				try:
					#globals.JEEDOM_COM.add_changes('devices::'+input['address'],input)
					globals.JEEDOM_COM.add_changes('devices::'+board['address'],board)
					globals.JEEDOM_COM.add_changes('input::',input)
					#globals.JEEDOM_COM.add_changes('board\' : \''+str(eqt.type) + '\' , \'address\' : \''+str(eqt.address),input)
					eqt.inputChanged = 2
				except Exception, e:
			...
			if eqt.routputChanged == 1:
				...
				try:
					#globals.JEEDOM_COM.add_changes('devices::'+routput['address'],routput)
					globals.JEEDOM_COM.add_changes('devices::'+board['address'],board)
					globals.JEEDOM_COM.add_changes('output::',routput)
					#globals.JEEDOM_COM.add_changes('devices::'+action['address:'],action)
					#globals.JEEDOM_COM.add_changes('board\' : \''+str(eqt.type) + '\' , \'address\' : \''+str(eqt.address),routput)
					eqt.routputChanged = 2
				except Exception, e:
